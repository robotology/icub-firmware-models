...
In this folder you can find the Simulink model to use for sending different current targets to AMCFOC.
The model can be used to send current targets to a AMCFOC board and receive the corresponding measured value. 
An interactive interface is provided to set the current reference and start the transmission.

**Note:** The PC running this model must be connected to the AMCFOC through a Vector transceiver, in order to take advantage of the *Vehicle Network toolbox*.
