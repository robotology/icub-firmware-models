...
function update_initial_actuators_config(dictionary, init_conf_dictionary_name, init_conf_data)
    ...
    if(exist(dd,'N_MOTORS'))
        numOfActuators = dd.getEntry('N_MOTORS').getValue;
    else
        disp("N_MOTORS was not found in the dictionary '" + ...
    ...
    if(length(init_conf_data) ~= numOfActuators)
        throw("N_MOTORS does not match the size of " + ...
            init_conf_data)
    end
    ...
    disp("Initial configuration of " + numOfActuators + " actuator(s) called '" + ...
        init_conf_dictionary_name + "' was saved successfully to dictionary '" + dictionary + "'")
end
